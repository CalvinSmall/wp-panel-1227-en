...
func FixWPConfigCredentials(webRoot, domain, dbName, dbUser, tablePrefix string) error {
	...
	// Replace DB_NAME (supports single and double quote formats)
	...
	if reNameSingle.MatchString(result) {
		...
		return fmt.Errorf("DB_NAME definition not found (path: %s). wp-config.php may be malformed or use unusual quoting", configPath)
	}

	// Replace DB_USER (supports single and double quote formats)
	...
	if reUserSingle.MatchString(result) {
		...
		return fmt.Errorf("DB_USER definition not found (path: %s). wp-config.php may be malformed or use unusual quoting", configPath)
	}
 ...
 * To add custom configuration, insert after the "/* Add any custom values" marker line below.
 *
 * @package WordPress
 */

// ** Database settings — do not modify below (managed by panel) ** //
define('DB_NAME', '%s');
define('DB_USER', '%s');
 ...
 *
 * Each site uses unique random keys, provided by the WordPress.org key generation service.
 * Replace manually with custom values if desired.
 */
%s
/**#@-*/
 ...
 *
 * To install multiple WordPress sites in one database, use a different prefix for each.
 ...
 * Debug mode
 *
 * Recommended for development, should be kept off in production.
...
/** Load WordPress settings and include files */
